...
function* addCredentialSaga(action) {
  try {
    ...
    else if(credentialData?.issuer && typeof credentialData.issuer === 'string'){
  ...
  if(Array.isArray(credential.type)){
    ...
    const credentials = yield call(SecureStore.getItemAsync, "credentials");
    if (!credentials) {
      const credentialsArray = [credential];
      yield call(SecureStore.setItemAsync, "credentials", JSON.stringify(credentialsArray));
    } else {
      let credentialsParsed = JSON.parse(credentials);
      credentialsParsed = [credential, ...credentialsParsed];
      yield call(SecureStore.setItemAsync, "credentials", JSON.stringify(credentialsParsed));
    }
...
function* removeCredentialSaga(action){
  ...
  if(!(index>=0))
    ...
    try {
      ...
      let credentialsParsed = JSON.parse(credentials);
      credentialsParsed.splice(index)
      yield call(SecureStore.setItemAsync, "credentials", JSON.stringify(credentialsParsed));
